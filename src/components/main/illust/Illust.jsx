...
export default function Illust() {
	...
	const handleCustomScroll = scroll => {
		const pathLength = 1510;
		// 해당 섹션의 기준점에 도달하기 전까지는 기존 값 고수
		if (scroll < 0) {
			pathEl.current.style.strokeDashoffset = pathLength;
		}
		// 섹션에 도달하는 순간부터 스크롤값 연동
		if (scroll >= 0) {
			let resultScroll = 0;
			pathLength - scroll * 4 < 0 ? (resultScroll = 0) : (resultScroll = pathLength - scroll * 4);
	...
	return (
		<div className='Illust myScroll' ref={refEl}>
			<div className='svgBox'>
				{/* viewBox='가로위치값 세로위치값 가로폭의비율 세로폭의비율' */}
